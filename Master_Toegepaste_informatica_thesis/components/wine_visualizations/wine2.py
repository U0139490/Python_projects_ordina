...
layout = html.Div(
    [
                    ...
                    [
                        ...
                        html.Div(
                            [  html.Div(
                                    ...
                                    html.Label([ dcc.Dropdown(id="wine_input2",
                                ...
                                html.Div(
                                    [ html.H5('Average Rating'),html.H6(id="well_text"),  html.Div(id='output_averageRating_wine2',style={"fontSize":"20px","margin":"auto","margin-top":"15%","textAlign":"center"})],
                                    id="wells",
                                    className="mini_container",
                                ),
                                html.Div(
                                    [html.H5('Winery'),html.H6(id="gasText"),  html.Div(id='output_wineryName_wine2',style={"fontSize":"20px","margin":"auto","margin-top":"15%","textAlign":"center"})],
                                    id="gas",
                                    className="mini_container",
                                ),
                                html.Div(
                                    [html.H5('Country'), html.H6(id="oilText"), html.Div(id='output_countryName_wine2',style={"fontSize":"20px","margin":"auto","margin-top":"15%","textAlign":"center"})],
                                    id="oil",
                                    className="mini_container",
                                ),
        ...
        html.Div(
                [
                        html.Div(
                        [dcc.Graph(id="graph1_wine2")],
                        className="pretty_container",id="graph1_wine2_div"),

                        html.Div(
                        [dcc.Graph(id="graph2_wine2")],
                        className="pretty_container",id="graph2_wine2_div"),
            ...
            # html.Div([                
            #     html.H2('SECTION 2',style={"color":"white", "fontWeight":"bold","padding":"5px"}),
                html.Div(
                    [
                        html.Div(
                            [dcc.Graph(id="barchart_negative_tab1")],
                            className="pretty_container five columns",id="div_barchar_sentiment"
                        ), 
                        html.Div(

                            [dcc.Graph(id="barchart_neutral_tab1")],
                            className="pretty_container five columns",id="div_barchar_sentiment"
                        ), 
                        html.Div(
                            [dcc.Graph(id="barchart_positive_tab1")],
                            className="pretty_container five columns",id="div_barchar_sentiment"
                        ), 
...
@app.callback(Output("graph1_wine2", "figure"),
             [Input("submit-button", "n_clicks")],
              [State("wine_input2", "value")])
...
@app.callback(Output("graph2_wine2", "figure"),
              [Input("submit-button", "n_clicks")],
              [State("wine_input2", "value")])
...
@app.callback([Output('output_averageRating_wine2', 'children'),
                Output('output_wineryName_wine2', 'children'),
                Output('output_countryName_wine2', 'children')],
             [Input("submit-button", "n_clicks")],
              [State("wine_input2", "value")])
...
# @app.callback(Output("graph5_tab1", "figure"),
#               [Input("submit-button", "n_clicks")],
#               [State("wine_input", "value")])
# def negativeBoxplot(n_clicks, input_value):
#      if len(input_value)>0: 
#         negativeWines = (wine_csv_transform.loc[ wine_csv_transform['Sentiment']== "negative"])
#         amount_pos = negativeWines.query('Sentiment == "negative"').Sentiment.count()

        
#         labels = ['Positive', 'Negative','Neutral']
#         values = []
#         graph_boxplot = go.Box( y=negativeWines['rating'],
#                         name = "negative",
#                         marker = dict(
#                         color =  "red")
#                     )          
...
@app.callback([Output("barchart_negative_tab1", "figure"),
                Output("barchart_neutral_tab1", "figure"),
                Output("barchart_positive_tab1", "figure")],
              [Input("submit-button", "n_clicks")],
              [State("wine_input2", "value")])
def update_barChart(n_clicks, input_value):
    if input_value is not None:
        specific_wine = (wine_csv_transform.loc[ wine_csv_transform['wine']== input_value])
        ...
        if len(negative_reviews) !=0:
            ...
            total_words_negative = sum(word_occurrence_negative)
            top_words_negative = word_occurrence_negative[:30]/total_words_negative

            barchart_tab1_negative = go.Bar(
                x=top_words_negative.values,
                y=top_words_negative.index,
                marker_color="#DC3220",
                name='Amount Reviews',
                orientation='h')    
        else:
        ...
        if len(positive_reviews) !=0:
            ...
            total_words_positive = sum(word_occurrence_positive)
            top_words_positive = word_occurrence_positive[:30]/total_words_positive

            barchart_tab1_positive = go.Bar(
                x=top_words_positive.values,
                y=top_words_positive.index,
                marker_color="#005AB5",
                name='Amount Reviews',
                orientation='h')  
        else:
        ...
        if len(neutral_reviews) !=0:
            ...
            total_words_neutral = sum(word_occurrence_neutral)
            top_words_neutral = word_occurrence_neutral[:30]/total_words_neutral

            barchart_tab1_neutral = go.Bar(
                x=top_words_neutral.values,
                y=top_words_neutral.index,
                marker_color="#FFC20A",
                name='Amount Reviews',
                orientation='h')  
        else:
        ...
        layout_graph_barchart_negative = dict(
                        title = "% Occurrence of characteristics In Negative Reviews ",
                        paper_bgcolor='rgba(0,0,0,0)',
                        plot_bgcolor='rgba(0,0,0,0)',
        ...
        layout_graph_barchart_positive = dict(
                        title = "% Occurrence of characteristics In Positive Reviews ",
                        paper_bgcolor='rgba(0,0,0,0)',
                         plot_bgcolor='rgba(0,0,0,0)',
                         yaxis=dict(autorange="reversed"),
                        hovermode='closest',
        ...
        layout_graph_barchart_neutral = dict(
                        title = "% Occurrence of characteristics In Neutral Reviews",
                        paper_bgcolor='rgba(0,0,0,0)',
                         plot_bgcolor='rgba(0,0,0,0)',
                         yaxis=dict(autorange="reversed"),
                        hovermode='closest',
        ...
        return  {
                "data": [barchart_tab1_negative],
                "layout": layout_graph_barchart_negative,
            },{
                "data": [barchart_tab1_neutral],
                "layout": layout_graph_barchart_neutral,
            },{
                "data": [barchart_tab1_positive],
                "layout": layout_graph_barchart_positive,
            },
...
# @app.callback(
#     Output("wine_input2", "options"),
#     [Input("wine_input2", "search_value")],
# )
